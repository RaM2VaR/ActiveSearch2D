...
parser.add_argument('--dataset', help='path to dataset', type=str,default='data_TGFF/data_single_TGFF1_norm_81.pt')
parser.add_argument('--max_iter', help='max iterations', type=int, default=10000)
parser.add_argument('--num_samples', help='number of unique solutions to be sampled in each iteration', type=int, default=512)
...
def load_model(graph_size=121, device = torch.device('cpu'), feature_scale = 1, pretrained_model_path = None, model='lstm', transformer_version='v2'):
    if model == 'lstm':
        mpnn_ptr =MpnnPtr(input_dim=graph_size, embedding_dim=graph_size + 7,
                   hidden_dim=graph_size + 7+args.hidd_dim, K=3, n_layers=args.n_layers, p_dropout=0, device=device, logit_clipping=False)
