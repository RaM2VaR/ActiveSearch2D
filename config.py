...
def argparser():
	...
	parser.add_argument('-t', '--city_t', metavar = 'T', type = int, default = 16, help = 'number of cities(nodes), time sequence, default: 20')
	parser.add_argument('-s', '--steps', metavar = 'S', type = int, default = 1000, help = 'training steps(epochs), default: 15000')
	parser.add_argument('-dimX','--dimX',metavar='X',type=int,default=4,help='3D mesh x dim, default=4')
	parser.add_argument('-dimY','--dimY',metavar='Y',type=int,default=4,help='3D mesh y dim, default=4')
	parser.add_argument('-dimZ','--dimZ',metavar='Z',type=int,default=2,help='3D mesh z dim (this work consider Z=2 only)')
	parser.add_argument('-dataPt','--dataPt',metavar='dataPt',type=str,required=True,help='3D traffic pattern in .pt extension')
	parser.add_argument('-dataTxt','--dataTxt',metavar='dataTxt',type=str,required=True,help='3D traffic pattern in .txt extension')
	# details
...
class Config():
	def __init__(self, **kwargs):	
		...
		self.task = '%s%d%d%d'%('mesh_', self.dimX,self.dimY,self.dimZ)
...
def pkl_parser():
	...
	parser.add_argument('-p', '--path', metavar = 'P', type = str, 
						default = 'Pkl/mesh_442.pkl', help = 'pkl file name')
	args = parser.parse_args()
	return args
