...
class Decoder_tsv(nn.Module):
    # module for single step of decoding process
    def __init__(self, input_dim, hidden_dim, n_layers, p):
        super(Decoder_tsv, self).__init__()
        self.output_dim = input_dim
        self.hidden_dim = hidden_dim
        self.n_layers = n_layers
        self.rnn = nn.LSTM(input_dim, hidden_dim, n_layers, dropout=p)
...
class PointerNet(nn.Module):
    ...
    def forward(self, input: Tensor, mask, num_samples=1):
        ...
        # predicted_tsvs     = torch.zeros(batch_size * num_samples, int(seq_len/8), dtype=torch.int64).to(self.device)
        # hidden_tsv  = hidden
        # cell_tsv    = cell 
        # encoder_outputs_tsv = encoder_outputs  
        # first input should be a part of model learnable parameters
        decoder_input = self.initial_decoder_input.repeat(batch_size, 1)
        # decoder_input_tsv = self.initial_decoder_input.repeat(batch_size, 1)
        # mask to be used while calculating attention weights
        mask_decoding = mask.clone()
        # mask_decoding_tsv = mask.clone()
        ...
        # loop for tsv prediction
        # tsv_num = int(seq_len/8)
        # for ds in range(tsv_num):
        #     output_tsv, (hidden_tsv, cell_tsv) = self.decoder_tsv(decoder_input_tsv, hidden_tsv, cell_tsv)
        #     logits_tsv = self.attn(output_tsv, encoder_outputs_tsv, mask_decoding_tsv)
        #     log_probs_tsv = F.log_softmax(logits_tsv, dim=1)
        #     if ds == 0:
        #         if self.decoding_type != 'sampling':
        #             if self.decoding_type == 'sampling-w/o-replacement':
        #                 scores_tsv = log_probs_tsv + gumbel_like(log_probs_tsv)
        #             elif self.decoding_type == 'greedy':
        #                 scores_tsv = log_probs_tsv
        #             _, selected_indices_tsv = torch.topk(scores_tsv, min(num_samples, scores.size(-1)), dim=-1)
        #             if num_samples > log_probs_tsv.size(1):
        #                 # pad second dimension with -1 so that the shape becomes (batch, num_samples)
        #                 selected_indices_tsv = F.pad(selected_indices_tsv, (0, num_samples - scores.size(1)), 'constant', -1)
        #             selected_indices_tsv = selected_indices_tsv.view(-1) 
        #         else:
        #             selected_indices_tsv = torch.multinomial(log_probs_tsv.exp(), num_samples, replacement=True).long().view(-1)
        #         log_probs_tsv = log_probs_tsv.repeat_interleave(num_samples, dim=0)
        #         # make log_probs -inf for the values which are -1 in selected_indices
        #         log_probs_tsv = log_probs_tsv.masked_fill((selected_indices == -1)\
        #             .unsqueeze(-1), float('-inf'))
        #         if self.decoding_type == 'sampling-w/o-replacement':
        #             scores_tsv = scores_tsv.repeat_interleave(num_samples, dim=0)
        #             scores_tsv = scores_tsv.masked_fill((selected_indices == -1)\
        #                 .unsqueeze(-1), float('-inf'))
        #         mask_decoding_tsv = mask_decoding_tsv.repeat_interleave(num_samples, dim=0)
        #         hidden_tsv = hidden_tsv.repeat_interleave(num_samples, dim=1)
        #         cell_tsv = cell_tsv.repeat_interleave(num_samples, dim=1)
        #         encoder_outputs_tsv = encoder_outputs_tsv.repeat_interleave(num_samples, dim=1)
        #     else:
        #         if self.decoding_type != 'sampling':
        #             if self.decoding_type == 'sampling-w/o-replacement':
        #                 scores_tsv, _ = gumbel_with_maximum(log_probs_tsv + log_probs_sum_tsv.unsqueeze(-1), g_log_probs)
                        
        #             elif self.decoding_type == 'greedy':
        #                 scores_tsv = log_probs_tsv + log_probs_sum_tsv.unsqueeze(-1) 
        #             scores_per_batch_tsv = scores_tsv.view(batch_size, -1)
        #             _, indices_buf_tsv = torch.topk(scores_per_batch_tsv, num_samples, dim=1)
        #             # indices_buf shape: (batch, min(num_samples, scores.size(1)))
        #             beams_buf_tsv = torch.div(indices_buf_tsv, tsv_num,rounding_mode='floor')
        #             indices_buf_tsv = indices_buf_tsv.fmod(tsv_num)
        #             selected_indices_tsv = indices_buf_tsv.view(-1)
        #             predicted_tsvs[:,:t] = rearrange(predicted_tsvs[:,:t], beams_buf_tsv)
        #             log_probs_sum_tsv = rearrange(log_probs_sum_tsv, beams_buf_tsv)
        #             mask_decoding_tsv = rearrange(mask_decoding_tsv, beams_buf_tsv)
        #             log_probs_tsv = rearrange(log_probs_tsv, beams_buf_tsv)
        #             hidden_tsv = rearrange(hidden_tsv, beams_buf_tsv)
        #             cell_tsv = rearrange(cell_tsv, beams_buf_tsv)
        #             if self.decoding_type == 'sampling-w/o-replacement':
        #                 scores_tsv = rearrange(scores_tsv, beams_buf_tsv)
        #         else:
        #             selected_indices_tsv = torch.multinomial(log_probs_tsv.exp(), 1).long().squeeze(1)
               
            
        #     predicted_tsvs[:, ds] = selected_indices_tsv
        #     gather_indices_tsv = selected_indices_tsv.unsqueeze(-1).clone()
        #     gather_indices_tsv[gather_indices_tsv == -1] = 0 
        #     curr_log_probs_tsv = log_probs_tsv.gather(1, gather_indices_tsv).squeeze(-1) \
        #         * mask.repeat_interleave(num_samples, dim=0)[:, ds]
        #     log_probs_sum_tsv += curr_log_probs_tsv
        #     if self.decoding_type == 'sampling-w/o-replacement':
        #         # gumbel perturbed log probabilities of partial sequences
        #         g_log_probs_tsv = scores_tsv.gather(1, gather_indices_tsv).squeeze(-1) \
        #             * mask.repeat_interleave(num_samples, dim=0)[:, ds]
        #     decoder_input_tsv = input.repeat_interleave(num_samples, dim=1)[gather_indices_tsv.squeeze(-1),torch.arange(batch_size * num_samples)]
        #     # update the mask_decoding to remove the pointed inputs
        #     mask_decoding_tsv.scatter_(1, gather_indices_tsv, 0)
