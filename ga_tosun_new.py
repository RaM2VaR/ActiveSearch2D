...
import torch
import math
from utils.utils import communication_cost_multiple_samples,LPNet_pred,communication_energy_multiple_samples,booksim_latency
...
def crossover(prtls):
    ...
    partner_idxs = np.random.permutation(prtls.shape[0])
    # partner_idxs2 = np.random.permutation(prtls.shape[0]//2)
    for i in range(prtls.shape[0]//2):
        # randomly choose a cut point
        cut_point = random.randint(1, prtls.shape[1] - 1)
        childs[2*i, :cut_point] = prtls[partner_idxs[2*i], :cut_point]
        childs[2*i, cut_point:] = prtls[partner_idxs[2*i+1], cut_point:]
        childs[2*i+1, :cut_point] = prtls[partner_idxs[2*i+1], :cut_point]
        childs[2*i+1, cut_point:] = prtls[partner_idxs[2*i], cut_point:]
...
class GA:
    def __init__(self, dataset_path, num_particles,\
        max_iterations, prtl_init_method="random", model_path=None, use_3d=False,max_graph_size=121):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', help="path to the data file", type=str,default='data_TGFF/data_single_TGFF1_norm_64.pt')
    ...
    # file1 = open("results/sim_results/GA_"+args.obj_fun+"_"+args.dataset[-16:-3]+'.csv','a')
    # file1.write(f'{args.dataset[-16:-3]},{cost:.2f} ,{(end_time-st_time):.2f},{bst_mapp}\n')
