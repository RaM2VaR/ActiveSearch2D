...
fit_fun = 'comm_cost'  # select the fitness function here
if fit_fun == 'LPNet':
...
root_train = 'data_tgff/multiple/train'   #change here
root_dev = 'data_tgff/multiple/test'      #change here 
...
if model == "lstm":
    mpnn_ptr = MpnnPtr(input_dim=max_graph_size, embedding_dim=max_graph_size + 7,
                   hidden_dim=max_graph_size + 7+128, K=3, n_layers=1, p_dropout=0, device=device, logit_clipping=False)
...
if saved_model_path:
    ...
    print(f'loading with partial trained model')
else:
    mpnn_ptr.apply(init_weights)
# %% initialize the training algorithm
if training_algorithm == 'init_pop':
    trainer = TrainerInitPop(mpnn_ptr,fit_fun, num_samples)
elif training_algorithm == 'pretrain':
    trainer = TrainerSR(mpnn_ptr,fit_fun, num_samples)
...
#     mpnn_ptr, tqdm(dev_dataloader, leave=False), distance_matrix_dict, beam_width,fit_fun) / len(dev_dataloader.dataset)
...
for epoch in range(num_epochs):
    ...
    # save the model
    f.write(f"{print_str}\n")
    torch.save(mpnn_ptr.state_dict(), per_epoch_save_folder /f'mpnn_ptr_CommCost_{training_algorithm}_{datetime_suffix}_{epoch + 1}.pt')
...
), model_save_folder / f'model_commCost_{training_algorithm}_{datetime_suffix}.pt')
